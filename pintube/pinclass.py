...
class Pintube(object):
    ...
    def get_youtube_data(self):
        ...
        self.youtube_data = current_user.info.youtube_playlists
    ...
    def copy_playlist_to(self, original_playlist_url, new_playlist_name):
        ...
        for id in original_feed_list_id:
            if (p_exist["exist"]):
                if id not in new_feed_list_id:
                    try:
                        self.add_video_to_playlist(id, p_exist["id"], 1)
                    except RequestError, error:
                        print error
                        print "Adding Video to the Already Existing Playlist Did Not Work"
